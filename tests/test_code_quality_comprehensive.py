...
class ComprehensiveCodeQualityValidator:
    ...
    def check_sys_path_duplication(self, file_path: Path) -> List[str]:
        ...
        try:
            ...
            sys_path_inserts = re.findall(r'sys\.path\.insert', content)
            
            if len(sys_path_inserts) > 1:
                issues.append(f"Multiple sys.path.insert calls found: {len(sys_path_inserts)}")
                ...
                if '"""' in line or "'''" in line:
                    if not in_string:
                        ...
                        if len(current_string) > 20:  # More than 20 lines
                            issues.append(f"Large multi-line string at line {i-len(current_string)+1}")
                        current_string = []
                elif in_string:
                    current_string.append(line)
    ...
    def validate_all_test_files(self):
        ...
        for test_file in test_files:
            ...
            duplicate_issues = self.check_duplicate_imports(test_file)
            file_issues.extend(duplicate_issues)
            
            # Check for unused imports
            unused_issues = self.check_unused_imports(test_file)
            file_issues.extend(unused_issues)
            
            # Check for sys.path duplication
            sys_path_issues = self.check_sys_path_duplication(test_file)
            file_issues.extend(sys_path_issues)
            
            # Check for large strings
            large_string_issues = self.check_large_strings(test_file)
...
def main():
    ...
    validator = ComprehensiveCodeQualityValidator()
