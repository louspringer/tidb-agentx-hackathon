            ...
            if re.match(r'^\s*def\s+test_\w+\s*\([^)]*\)\s*->\s*None\s*:$', line):
                ...
                for body_line in func_body:
                    if re.search(r'^\s*return\s+[^#\n]+', body_line.strip()):
                        if not re.match(r'^\s*return\s*$', body_line.strip()) and \
                           not re.match(r'^\s*return\s+None\s*$', body_line.strip()):
                ...
                if has_return_value:
                    ...
                    for body_line in func_body:
                        if re.search(r'^\s*return\s+[^#\n]+', body_line.strip()):
                            ...
                            modified_body.append(' ' * indent + '# Removed return statement')
                            changes_made = True
                        else:
...
def main() -> None:
    ...
    if total_fixed > 0:
        ...
        if result.stdout:
            return_value_errors = [line for line in result.stdout.split('\n') if 'No return value expected' in line]
