        ...
        try:
            ...
            if not self.connect():
                return ConversionResult(
                    success=False, errors=["Failed to connect to Neo4j"]
                )
            ...
            logger.info(
                f"✅ Conversion completed: {total_nodes} nodes, {relationships} relationships"
            )
        ...
        with self.driver.session() as session:
            ...
            for file_path, file_data in ast_models.items():
                for func in file_data.get("functions", []):
                    ...
                    session.run(
                        cypher,
                        {"file_path": file_path, "func_name": func.get("name", "")},
                    )
                    count += 1
            ...
            for file_path, file_data in ast_models.items():
                for cls in file_data.get("classes", []):
                    ...
                    session.run(
                        cypher,
                        {"file_path": file_path, "class_name": cls.get("name", "")},
                    )
                    count += 1
            ...
            for file_path, file_data in ast_models.items():
                for imp in file_data.get("imports", []):
                    ...
                    session.run(
                        cypher,
                        {"file_path": file_path, "module": imp.get("module", "")},
                    )
                    count += 1
