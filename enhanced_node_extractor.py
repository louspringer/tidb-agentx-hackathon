#!/usr/bin/env python3
"""
Enhanced Node Extractor: Fix order and duplication issues

This extractor:
1. Preserves order with position attributes
2. Deduplicates nodes properly
3. Handles Python's top-to-bottom processing
4. Maintains proper file structure
"""
...
class EnhancedNodeExtractor:
    """Enhanced node extractor with order preservation and deduplication."""
    ...
    def extract_from_file(self, file_path: str) -> List[CodeNode]:
        """Extract nodes with proper order and deduplication."""
        ...
        import_nodes = self._extract_imports_ordered(tree, file_path, context)
        nodes.extend(import_nodes)
        
        # Extract classes and functions in order
        class_function_nodes = self._extract_classes_and_functions_ordered(tree, file_path, context)
        nodes.extend(class_function_nodes)
        
        # Extract constants and other top-level elements
        constant_nodes = self._extract_constants_ordered(tree, file_path, context)
    ...
    def _extract_imports_ordered(self, tree: ast.AST, file_path: str, context: str) -> List[CodeNode]:
        """Extract imports in order with deduplication."""
        nodes = []
        seen_imports = set()
    ...
    def _extract_classes_and_functions_ordered(self, tree: ast.AST, file_path: str, context: str) -> List[CodeNode]:
        """Extract classes and functions in order with deduplication."""
        nodes = []
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.ClassDef)):
                ...
                # Skip if we've seen this exact definition
                if definition_code in seen_definitions:
                    continue
                
                seen_definitions.add(definition_code)
                
                # Create unique ID
                if isinstance(node, ast.FunctionDef):
                    node_id = f"function_{node.name}_{hash(definition_code)}"
                    node_type = "function"
                else:
                    node_id = f"class_{node.name}_{hash(definition_code)}"
                    node_type = "class"
                ...
                code_node = CodeNode(
                    ...
                    metadata={
                        "function_name" if node_type == "function" else "class_name": node.name,
    ...
    def _extract_constants_ordered(self, tree: ast.AST, file_path: str, context: str) -> List[CodeNode]:
        """Extract constants and other top-level elements."""
        nodes = []
        seen_constants = set()
...
def main():
    """Demonstrate enhanced node extraction."""
    logger.info("🚀 Starting Enhanced Node Extraction")
    
    extractor = EnhancedNodeExtractor()
    ...
    if Path(test_file).exists():
        ...
        # Save enhanced nodes
        extractor.save_extracted_nodes("enhanced_extracted_nodes.json")
