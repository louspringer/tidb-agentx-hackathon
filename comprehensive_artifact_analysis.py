...
class ArtifactAnalyzer:
        ...
        for root, dirs, files in os.walk(self.project_root):
            ...
            for file in files:
                ...
                try:
                    ...
                    if 'mcp_integration' in str(file_path) or 'ghostbusters_gcp' in str(file_path) or 'mdc_generator' in str(file_path):
                    ...
                    if file_type == 'python':
                        ...
                        if not ast_result['success']:
                            artifact.issues.append(f"AST parsing failed: {ast_result.get('error', 'Unknown error')}")
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        python_issues = [a for a in self.artifacts if a.file_type == 'python' and a.issues]
        ...
        report = {
            'summary': {
                ...
                'coverage_percentage': (self.analysis.artifacts_traced / self.analysis.total_artifacts * 100) if self.analysis.total_artifacts > 0 else 0
            },
            'domain_analysis': {
            ...
            'requirements_analysis': {
                ...
                'total_requirements': len(self.model_data.get('requirements_traceability', [])),
                'traced_requirements': len(set().union(*[a.requirements_traced for a in self.artifacts]))
            },
            'python_analysis': {
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if self.analysis.artifacts_untraced > 0:
            recommendations.append(f"🔍 {self.analysis.artifacts_untraced} artifacts are not traced to any domain. Consider adding domain patterns for these files.")
        
        if self.analysis.missing_domains:
            recommendations.append(f"📁 Domains defined in model but no artifacts found: {', '.join(self.analysis.missing_domains)}")
        
        if self.analysis.missing_requirements:
            recommendations.append(f"📋 {len(self.analysis.missing_requirements)} requirements are not traced to any artifacts. Consider implementing these requirements.")
        
        # Python analysis recommendations
        if self.analysis.ast_parsing_failures > 0:
            recommendations.append(f"🐍 {self.analysis.ast_parsing_failures} Python files failed AST parsing. Review these files for syntax issues.")
        
        # Large file recommendations
        large_files = [a for a in self.artifacts if a.size_bytes > 1000000]
        if large_files:
            recommendations.append(f"📦 {len(large_files)} files are larger than 1MB. Consider if these should be in version control.")
...
def main():
    ...
    analysis = analyzer.analyze_model_coverage()
    ...
    if report['domain_analysis']['missing_domains']:
        print(f"Missing domains: {', '.join(report['domain_analysis']['missing_domains'])}")
    
    # Print requirements analysis
    print("\n📋 REQUIREMENTS ANALYSIS")
    print("=" * 40)
    print(f"Total Requirements: {report['requirements_analysis']['total_requirements']}")
    print(f"Traced Requirements: {report['requirements_analysis']['traced_requirements']}")
    
    if report['requirements_analysis']['missing_requirements']:
        print(f"Missing requirements: {len(report['requirements_analysis']['missing_requirements'])}")
