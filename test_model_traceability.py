...
from project_model import ProjectModel, DomainConfig
...
def test_requirement_1_cloudformation_detection():
    ...
    assert "!Sub" in cloudformation_config.content_indicators, "Content indicator missing"
    assert "cfn-lint" in cloudformation_config.linter, "Wrong linter selected"
    
    # Test result
    assert result.detected_domain == "cloudformation", f"Wrong domain: {result.detected_domain}"
...
def test_requirement_5_confidence_scoring():
    ...
    assert cf_result.confidence > 0.6, f"Low confidence for CloudFormation: {cf_result.confidence}"
    
    # Test Python file
    py_result = model.analyze_file("project_model.py")
    assert py_result.confidence > 0.6, f"Low confidence for Python: {py_result.confidence}"
