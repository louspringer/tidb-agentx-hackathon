#!/usr/bin/env python3
"""
Fixed Projection System: Use improved extraction with proper order

This system:
1. Uses the fixed node extractor
...
from fixed_node_extractor import FixedNodeExtractor


class FixedProjectionSystem:
    """Fixed projection system with proper order preservation."""
    
    def __init__(self):
        self.extractor = FixedNodeExtractor()
    ...
    def test_fixed_projection(self, test_files: List[str]) -> Dict[str, Any]:
        """Test fixed projection on multiple files."""
        results = {}
        ...
        for file_path in test_files:
            ...
            print(f"\n🔍 Testing fixed projection: {file_path}")
    ...
    def save_projected_file(self, file_path: str, projected_content: str) -> None:
        ...
        output_path = Path("fixed_projected_artifacts") / file_path
...
def main():
    """Test fixed projection system."""
    print("🚀 Fixed Projection System Test")
    print("=" * 60)
    
    system = FixedProjectionSystem()
    ...
    # Run tests
    results = system.test_fixed_projection(test_files)
