...
def test_live_llm_call_with_credentials():
    ...
    if "error" not in result:
        ...
        has_questions = (
            "questions" in result or 
            "probing_questions" in result or
            any(key.endswith("questions") for key in result.keys())
        )
        assert has_questions, f"Expected questions in result, got: {result}"
        
        # Get the questions list regardless of key name
        questions_key = next((key for key in result.keys() if key.endswith("questions")), None)
        if questions_key:
