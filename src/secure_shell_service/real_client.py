...
from secure_shell_pb2 import CommandRequest, HealthRequest
...
class MockSecureShellServiceStub:
    ...
    async def ExecuteCommand(self, request, timeout=None):
...
class RealSecureShellClient:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self.stub = MockSecureShellServiceStub(self.channel)  # Will be real stub later
            
            # Test connection with health check
            health = await self.health_check()
            if health.get('status') == 'healthy':
                self.logger.info(f"✅ Connected to secure shell service at {self.host}:{self.port}")
                return True
            else:
    ...
    async def execute_command(
        ...
        try:
            ...
            response = await self.stub.ExecuteCommand(request, timeout=timeout)
