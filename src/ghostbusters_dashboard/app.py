...
subscription_path = subscriber.subscription_path("ghostbusters-project", "analysis-updates-sub")
...
def get_user_analyses(user_id: str, limit: int = 50) -> List[Dict[str, Any]]:
    ...
    try:
        docs = db.collection("ghostbusters_results").where(
            "user_id", "==", user_id
        ).order_by(
...
def get_global_metrics() -> Dict[str, Any]:
    ...
    try:
        ...
        recent_analyses = list(db.collection("ghostbusters_results").where(
            "timestamp", ">=", week_ago
        ).stream())
        
        # Calculate metrics
        total_delusions = sum(len(doc.to_dict().get("delusions_detected", [])) for doc in recent_analyses)
        total_recoveries = sum(len(doc.to_dict().get("recovery_actions", [])) for doc in recent_analyses)
        avg_confidence = sum(doc.to_dict().get("confidence_score", 0) for doc in recent_analyses) / len(recent_analyses) if recent_analyses else 0
        avg_processing_time = sum(doc.to_dict().get("processing_time", 0) for doc in recent_analyses) / len(recent_analyses) if recent_analyses else 0
...
def main():
    ...
    page = st.sidebar.selectbox(
        "Choose a page",
        ["Overview", "My Analyses", "Analysis Details", "Real-time Updates", "Settings"]
    )
        ...
        for analysis in user_analyses[:5]:
            with st.container():
                ...
                with col2:
                    ...
                    st.markdown(f"<span style='color: {confidence_color}; font-weight: bold;'>{analysis['confidence_score']:.2f}</span>", unsafe_allow_html=True)
...
def show_my_analyses(user_id: str):
    ...
    with col2:
        confidence_filter = st.selectbox("Confidence", ["All", "High (>0.8)", "Medium (0.6-0.8)", "Low (<0.6)"])
    ...
    if analyses:
        for analysis in analyses:
            with st.expander(f"{analysis['project_path']} - {analysis['analysis_id'][:8]}..."):
                ...
                col1, col2 = st.columns(2)
                with col1:
                    if st.button("View Details", key=f"details_{analysis['analysis_id']}"):
                        st.session_state.selected_analysis = analysis['analysis_id']
                        st.rerun()
                
                with col2:
                    if st.button("Download Report", key=f"download_{analysis['analysis_id']}"):
...
def show_analysis_details():
    ...
    if analysis_id:
        ...
        if details:
            ...
            with col1:
                st.metric("Confidence Score", f"{details.get('confidence_score', 0):.2f}")
            
            with col2:
                st.metric("Delusions Detected", len(details.get("delusions_detected", [])))
            ...
            if delusions:
                for i, delusion in enumerate(delusions):
                    with st.expander(f"Delusion {i+1}: {delusion.get('type', 'Unknown')}"):
                        st.write(f"**File:** {delusion.get('file', 'Unknown')}")
                        st.write(f"**Description:** {delusion.get('description', 'No description')}")
...
def show_real_time_updates():
    ...
    st.info("Real-time updates would be implemented with WebSocket connections to Pub/Sub.")
    
    # Simulated updates
    updates = [
        {"analysis_id": "abc123", "progress": 25, "message": "Initializing agents..."},
        {"analysis_id": "def456", "progress": 50, "message": "Running security analysis..."},
