...
class MDCFile:
    ...
    def from_file(cls, file_path: Path) -> "MDCFile":
        ...
        return cls(frontmatter=frontmatter, content=content, file_path=file_path)
    ...
    def create_rule(
        ...
        frontmatter = MDCFrontmatter(
            description=description, globs=globs, always_apply=always_apply
        )

        return cls(frontmatter=frontmatter, content=content, file_path=file_path)

    def validate(self) -> None:
        """Validate the MDC file"""
        if not self.frontmatter.description:
            raise ValueError("Description is required")
        if not self.frontmatter.globs:
            raise ValueError("At least one glob pattern is required")
...
def main() -> None:
    ...
    parser.add_argument(
        "--generate", action="store_true", help="Generate standard rules"
    )
    parser.add_argument(
        "--validate", action="store_true", help="Validate all .mdc files"
    )
    parser.add_argument(
        "--base-dir", type=Path, default=Path("."), help="Base directory"
    )
