...
class FixSynthesis(BaseModel):
    ...
    stakeholder_impacts: Dict[str, str] = Field(
        description="Impact on each stakeholder"
    )
...
"""
        ...
        messages = [
            SystemMessage(
                content="You are an expert technical architect specializing in synthesizing diverse technical findings into actionable, prioritized solutions."
            ),
    ...
    def calculate_stakeholder_impact_matrix(
        ...
        return pd.DataFrame(
            impact_matrix, index=[fix.fix_title for fix in fixes], columns=stakeholders
        )

    def create_synthesis_visualizations(
        self, fixes: List[FixSynthesis], impact_matrix: pd.DataFrame
    ):
        ...
        bars = ax3.bar(
            categories, counts, color=plt.cm.Set3(np.linspace(0, 1, len(categories)))
        )
        ...
        bars = ax4.bar(
            range(len(fixes)),
            timelines,
            color=plt.cm.Pastel1(np.linspace(0, 1, len(fixes))),
        )
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "synthesis_analysis.png", dpi=300, bbox_inches="tight"
        )
        plt.savefig(
            self.output_dir / "synthesis_analysis.svg",
            format="svg",
            bbox_inches="tight",
        )
        plt.close()

    def generate_prioritized_report(
        self, fixes: List[FixSynthesis], impact_matrix: pd.DataFrame
    ):
...
def main() -> None:
    ...
    if results:
        ...
        print(
            f"   Average Priority: {sum(f.priority_score for f in results['fixes'])/len(results['fixes']):.2f}"
        )
        print(
            f"   High ROI Fixes: {sum(1 for f in results['fixes'] if f.estimated_roi == 'High')}"
        )
        print(
            f"   Low Effort Fixes: {sum(1 for f in results['fixes'] if f.implementation_effort == 'Low')}"
        )

        print("\n📋 Top 3 Prioritized Fixes:")
        sorted_fixes = sorted(
            results["fixes"], key=lambda x: x.priority_score, reverse=True
        )
        for i, fix in enumerate(sorted_fixes[:3], 1):
            print(
                f"   {i}. {fix.fix_title} (Priority: {fix.priority_score:.2f}, ROI: {fix.estimated_roi})"
            )
