...
class BlindSpotFinding(BaseModel):
    ...
    category: str = Field(
        description="Category: security, performance, ux, code_quality, devops"
    )
...
class LangGraphDiversityOrchestrator:
    ...
    def create_llm_client(self, agent: DiversityAgent) -> Any:
        ...
        if "openai" in agent.api_key_env.lower():
            return ChatOpenAI(
                model=agent.model, temperature=agent.temperature, api_key=api_key
            )
    ...
    def analyze_with_agent(
        self, agent: DiversityAgent, context: str
    ) -> DiversityAnalysis:
...
"""
            ...
            try:
                ...
                findings: List[Any] = [
                    BlindSpotFinding(**finding) for finding in findings_data
                ]
                ...
                avg_confidence = (
                    sum(confidence_scores.get(f.confidence, 0.5) for f in findings)
                    / total_findings
                    if findings
                    else 0
                )
    ...
    def calculate_diversity_metrics(
        ...
        overlap_rate = (
            1 - (unique_questions / total_findings) if total_findings > 0 else 0
        )
        ...
        return {
            ...
            "findings_by_category": pd.DataFrame(all_findings)
            .groupby("category")
            .size()
            .to_dict(),
            "findings_by_agent": pd.DataFrame(all_findings)
            .groupby("agent")
            .size()
            .to_dict(),
            "confidence_distribution": pd.DataFrame(all_findings)["confidence"]
            .value_counts()
            .to_dict(),
        }

    def create_visualizations(
        self, analyses: List[DiversityAnalysis], metrics: Dict[str, Any]
    ):
        ...
        if metrics["confidence_distribution"]:
            ...
            bars3 = ax4.bar(
                conf_levels,
                conf_counts,
                color=colors_conf[: len(conf_levels)],
                alpha=0.7,
            )
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "diversity_overview.png", dpi=300, bbox_inches="tight"
        )
        plt.savefig(
            self.output_dir / "diversity_overview.svg",
            format="svg",
            bbox_inches="tight",
        )
        plt.close()
    ...
    def create_network_graph(self, analyses: List[DiversityAnalysis]) -> None:
        ...
        for i, analysis1 in enumerate(analyses):
            for j, analysis2 in enumerate(analyses):
                if i < j:
                    ...
                    if overlap > 0:
                        G.add_edge(
                            analysis1.agent_name,
                            analysis2.agent_name,
                            weight=overlap,
                            overlap=overlap,
                        )
        ...
        plt.savefig(
            self.output_dir / "agent_network.svg", format="svg", bbox_inches="tight"
        )
        plt.close()

    def create_findings_analysis(
        self, analyses: List[DiversityAnalysis], metrics: Dict[str, Any]
    ):
        ...
        ax4.set_title(
            "Findings Distribution by Category", fontsize=14, fontweight="bold"
        )

        plt.tight_layout()
        plt.savefig(
            self.output_dir / "findings_analysis.png", dpi=300, bbox_inches="tight"
        )
        plt.savefig(
            self.output_dir / "findings_analysis.svg", format="svg", bbox_inches="tight"
        )
        plt.close()

    def generate_pdf_report(
        self, analyses: List[DiversityAnalysis], metrics: Dict[str, Any], context: str
    ):
        """Generate comprehensive PDF report"""

        doc = SimpleDocTemplate(
            str(self.output_dir / "diversity_analysis_report.pd"), pagesize=letter
        )
        ...
        for analysis in analyses:
            if analysis.findings:
                story.append(
                    Paragraph(f"<b>{analysis.agent_name}</b>", styles["Heading3"])
                )

                for i, finding in enumerate(
                    analysis.findings[:3], 1
...
"""
        ...
        markdown_content = jinja_template.render(
            analyses=analyses, metrics=metrics, context=context, agents=self.agents
        )
...
def main() -> None:
    ...
    print(
        f"   Agent Coverage: {results['metrics']['agent_coverage']}/{len(orchestrator.agents)}"
    )
