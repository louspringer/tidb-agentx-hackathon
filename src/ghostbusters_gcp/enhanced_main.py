...
def check_rate_limit(user_id: str) -> bool:
    ...
    try:
        ...
        if recent_requests.exists:
            ...
            if last_reset and datetime.fromisoformat(last_reset) < datetime.now() - timedelta(hours=1):
                request_count = 0
...
def ghostbusters_analyze_enhanced(request):
    ...
    try:
        ...
        logger.info("Starting enhanced Ghostbusters analysis for project: %s", project_path)
        ...
        logger.info("Enhanced analysis completed successfully. Confidence: %s", result.confidence_score)
