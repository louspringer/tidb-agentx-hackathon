...
class ImprovedProjectionSystem:
    ...
    def _extract_with_improvements(self, tree: ast.AST, file_path: str) -> List[CodeNode]:
        ...
        class_nodes = self._extract_classes_with_methods_improved(tree, file_path, context)
        nodes.extend(class_nodes)
        
        # Extract standalone functions (not class methods)
        function_nodes = self._extract_standalone_functions_improved(tree, file_path, context)
        nodes.extend(function_nodes)
        
        logger.info(f"✅ Improved extraction: {len(nodes)} nodes from {file_path}")
        return nodes
    
    def _extract_imports_improved(self, tree: ast.AST, file_path: str, context: str) -> List[CodeNode]:
        """Extract imports with better handling."""
        nodes = []
        seen_imports = set()
    ...
    def _extract_constants_improved(self, tree: ast.AST, file_path: str, context: str) -> List[CodeNode]:
    ...
    def _extract_classes_with_methods_improved(self, tree: ast.AST, file_path: str, context: str) -> List[CodeNode]:
    ...
    def _extract_standalone_functions_improved(self, tree: ast.AST, file_path: str, context: str) -> List[CodeNode]:
...
def main():
    ...
    if Path(test_file).exists():
        ...
        if projected_content:
            ...
            print(f"✅ Saved improved projection to improved_projection.py")
